...
Display battery percentages that macOS knows but does not expose through the app's existing public sources. The immediate target is the connected `Keychron K1 Max`, whose percentage appears in macOS while `IOBluetoothDevice`, I/O Registry, `system_profiler`, and CoreBluetooth's standard Battery Service do not return it.
